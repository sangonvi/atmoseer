...
STATION_SYSTEM_CONFIG = {
    ...
    "websirene": {
        "ids": set(globals.WEBSIRENE_STATION_IDS),
        "data_dir": globals.WEBSIRENE_DATA_DIR,
        "config_path": STATION_SYSTEM_CONFIG_DIR / "websirene.json",
        "raw_format": "csv", 
    },
}
...
def apply_scaling(df: pd.DataFrame, scaler_config: dict) -> pd.DataFrame:
    ...
    if scaler_type == "minmax":
        ...
        if feature_range == [0.0, 1.0] or tuple(feature_range) == (0.0, 1.0):
            return utils.min_max_normalize(df)
...
def preprocess_ws(ws_id, ws_filename, output_folder, station_system, raw_format="parquet"):
    ...
    if raw_format == "csv":
        try:
            df = pd.read_csv(ws_filename, parse_dates=["datetime"])
        except (FileNotFoundError, pd.errors.EmptyDataError) as e:
            logging.error(f"Failed to read CSV {ws_filename}: {e}")
            return 
    elif raw_format == "parquet":
        df = pd.read_parquet(ws_filename)
    else:
        logging.error(f"Unknown raw file format '{raw_format}'. Aborting.")
        return 
    ...
    df = utils.add_datetime_index(ws_id, df)
    logging.info(df.head())
    logging.info("Done!\n")
    ...
    df = utils.get_dataframe_with_selected_columns(df, column_names)
    df = utils.rename_dataframe_column_names(df, column_name_mapping)
    logging.info(df.head())
    logging.info("Done!\n")
    ...
    if not (predictor_names and target_name):
        variables = utils.get_relevant_variables(ws_id)
        if not variables:
            raise ValueError(
    ...
    if df.empty:
        logging.warning(f"No observations remaining for {ws_id} after dropping null target '{target_name}'. Aborting.")
        return
    ...
    if add_wind_features:
        ...
        df = utils.add_wind_related_features(ws_id, df)
        logging.info(df.head())
        logging.info("Done!\n")
    else:
    ...
    if add_hour_features:
        ...
        df = utils.add_hour_related_features(df)
        logging.info(df.head())
        logging.info("Done!\n")
    else:
    ...
    filename_and_extension = utils.get_filename_and_extension(ws_filename)
...
def main(argv):
    ...
    if station_system:
        ...
        if station_system == 'websirene':
            try:
                # Ensure the websirene ID is treated as a string
                station_id = str(int(station_id)) 
            except ValueError:
                parser.error(f"Station ID '{station_id}' for websirene must be numeric.")
        
        system_config = STATION_SYSTEM_CONFIG[station_system]
        
        # Compare IDs as strings for consistency
        if str(station_id) not in {str(sid) for sid in system_config["ids"]}:
            parser.error(
        ...
        for system_name, system_config in STATION_SYSTEM_CONFIG.items():
            # Compare IDs as strings for consistency
            if str(station_id) in {str(sid) for sid in system_config["ids"]}:
    ...
    system_config_full = STATION_SYSTEM_CONFIG[station_system]
    raw_format = system_config_full.get("raw_format", "parquet") # Default is parquet
    
    # Build the file name with the correct extension
    ws_filename = str(ws_data_dir) + str(station_id) + f".{raw_format}"
    
    # Check if the input file exists
    if not Path(ws_filename).exists():
        logging.error(f"Input file not found: {ws_filename}")
        parser.error(f"Input file not found: {ws_filename}")
    
    preprocess_ws(
        ws_id=station_id,
        ws_filename=ws_filename,
        output_folder=str(ws_data_dir),
        station_system=station_system,
