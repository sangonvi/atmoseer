...
def get_frame(data, extent):
    ...
    # Plot the image
    img = ax.imshow(
        data, origin="upper", extent=img_extent, cmap=colormap, animated=True
    )
...
def gen_animation(
    ...
    timestamps = generate_timestamps(
        initial_timestamp, final_timestamp, interval_in_minutes=10
    )
    ...
    img = ax.imshow(
        images[0], interpolation="none", extent=img_extent, cmap="gray", origin="upper"
    )
    ...
    plt.colorbar(
        img,
        label=f"Profundidade das nuvens - {initial_timestamp[:4]}-{initial_timestamp[4:6]}-{initial_timestamp[6:8]}",
        extend="both",
    ...
    ani = animation.FuncAnimation(
        fig, update_frame, frames=num_images, fargs=(img, images), blit=True
    )
...
gen_animation(
    initial_timestamp="202310310000",
