...
class GOES:
    ...
    def __str__(self):
        ...
        for key in list(self.__dict__.keys()):
            if isinstance(self.__dict__[key], np.ndarray):
                var.append(
                    "   {:29} ({}) {}".format(
                        key,
                        ", ".join(np.array(self.__dict__[key].shape).astype("str")),
                        str(self.__dict__[key].dtype),
                    )
                )
        ...
        return "\n".join(
            [str(self.__class__)] + [""] + ["Keys:"] + attr + classes + var + [""]
        )
    ...
    def refl_fact_to_refl(self, Lons, Lats, MinCosTheta=0.0, fmt=np.float32):
        ...
        if self.long_name == "ABI L2+ Cloud and Moisture Imagery reflectance factor":
            try:
                ...
                dict_Field = {
                    "long_name": "ABI L2+ Cloud and Moisture Imagery reflectance",
                    "standard_name": "toa_lambertian_equivalent_albedo",
                    "units": 1,
                    "undef": np.nan,
                    "axis": "YX",
                    "t": self.t,
                    "time_bounds": self.time_bounds,
                    "pixels_limits": self.pixels_limits,
                    "dimensions": ("y", "x"),
                    "data": self.data
            ...
            try:
                ...
                if "units" in parameter.ncattrs():
                    ...
                    if "long_name" in parameter.ncattrs():
                        if (
                            "seconds since" in parameter.long_name
                            or "time" in parameter.long_name
                        ):
                            if "(" and ")" in parameter.long_name:
                                units = (
                                    "seconds since "
                                    + re.findall(r"\(.+?\)", parameter.long_name)[0][
                                        1:-1
                                    ]
                                )
                            else:
        ...
        try:
            ...
            try:
                ...
                if isinstance(domain, list) or isinstance(domain, np.ndarray):
                    ...
                    Lons, Lats = get_lonlat(
                        X[::delta_index].astype(fmt),
                        Y[::delta_index].astype(fmt),
                        PlatformID,
                        SatLon,
                        SatHeight,
                        SatSweep,
                    ...
                    xpixmin, xpixmax, ypixmin, ypixmax = find_pixels_of_region(
                        Lons, Lats, LLLon, URLon, LLLat, URLat
                    )
                    del Lons, Lats
                    ...
                    Lons, Lats = get_lonlat(
                        X[xini : xfin + 1].astype(fmt),
                        Y[yini : yfin + 1].astype(fmt),
                        PlatformID,
                        SatLon,
                        SatHeight,
                        SatSweep,
                    ...
                    Limits = np.array(
                        [xini + xpixmin, xini + xpixmax, yini + ypixmin, yini + ypixmax]
                    )
                    ...
                    if lonlat == "center":
                        ...
                        dict_Lons = Lons
                        dict_Lats = Lats
                        dict_Lons.data = np.ascontiguousarray(
                            Lons.data[ypixmin : ypixmax + 1, xpixmin : xpixmax + 1],
                            dtype=fmt,
                        )
                        dict_Lats.data = np.ascontiguousarray(
                            Lats.data[ypixmin : ypixmax + 1, xpixmin : xpixmax + 1],
                            dtype=fmt,
                        )

                    elif lonlat == "corner":
                        Lons, Lats = get_lonlatcorner(
                            X[xini + xpixmin : xini + xpixmax + 1].astype(fmt),
                            Y[yini + ypixmin : yini + ypixmax + 1].astype(fmt),
                            PlatformID,
                            SatLon,
                            SatHeight,
                            SatSweep,
                ...
                elif isinstance(domain_in_pixels, list) or isinstance(
                    domain_in_pixels, np.ndarray
                ):
                    ...
                    if lonlat == "center":
                        Lons, Lats = get_lonlat(
                            X[xpixmin : xpixmax + 1].astype(fmt),
                            Y[ypixmin : ypixmax + 1].astype(fmt),
                            PlatformID,
                            SatLon,
                            SatHeight,
                            SatSweep,
...
class open_mfdataset:
    ...
    def dimension(self, parameter):
        ...
        return GOES(
            {"name": np.array(mfparameter_name), "size": np.array(mfparameter_size)}
        )
                ...
                try:
                    ...
                    if "units" in mfparameter.ncattrs():
                        ...
                        if "long_name" in mfparameter.ncattrs():
                            if (
                                "seconds since" in mfparameter.long_name
                                or "time" in mfparameter.long_name
                            ):
                                if "(" and ")" in mfparameter.long_name:
                                    units = (
                                        "seconds since "
                                        + re.findall(r"\(.+?\)", mfparameter.long_name)[
                                            0
                                        ][1:-1]
                                    )
                                else:
                ...
                dict.update(
                    {
                        "dimensions": (
                            "number_of_files",
                            getattr(mfparameter, "dimensions")[0],
                        )
                    }
                )
            else:
...
def get_lonlat(X, Y, PlatformID, SatLon, SatHeight, SatSweep, fmt=np.float32):
    ...
    Lons = np.where(
        (Lons >= -360.0) & (Lons <= 360.0) & (Lats >= -90.0) & (Lats <= 90.0),
        Lons,
        -999.99,
    ).astype(fmt)
    Lats = np.where(
...
def get_lonlatcorner(X, Y, PlatformID, SatLon, SatHeight, SatSweep, fmt=np.float32):
    ...
    dict_Lons = {
        "long_name": "Longitude of corners of pixels",
        "standard_name": "pixels corners longitude",
        "units": "degrees_east",
        "undef": -999.99,
        "axis": "YX",
        "dimensions": ("y", "x"),
    ...
    dict_Lats = {
        "long_name": "Latitude of corners of pixels",
        "standard_name": "pixels corners latitude",
        "units": "degrees_north",
        "undef": -999.99,
        "axis": "YX",
        "dimensions": ("y", "x"),
...
def corner_size_to_center_size(Field):
    ...
    Field = np.concatenate(
        (Field[:, 0 : int(nx / 2.0)], Field[:, int(nx / 2.0) + 1 :]), axis=1
    )
    Field = np.concatenate(
        (Field[0 : int(ny / 2.0), :], Field[int(ny / 2.0) + 1 :, :]), axis=0
    )
    return Field
...
def midpoint_in_x(Field, fmt=np.float32):
    ...
    right = np.column_stack(
        (Field[:, 1:], np.full([Field.shape[0], 1], -999.99, dtype=fmt))
    )
    left = np.column_stack(
        (np.full([Field.shape[0], 1], -999.99, dtype=fmt), Field[:, :-1])
    )
    left2 = np.column_stack(
        (np.full([Field.shape[0], 2], -999.99, dtype=fmt), Field[:, :-2])
    )

    midpoint = np.where(
        (Field > -400.0) & (left < -400.0), Field - (right - Field) / 2.0, -999.99
    )
    midpoint = np.where(
        (Field > -400.0) & (left > -400.0), (left + Field) / 2.0, midpoint
    )
    midpoint = np.where(
        (Field < -400.0) & (left > -400.0), left + (left - left2) / 2.0, midpoint
    )
    return midpoint
...
def midpoint_in_y(Field, fmt=np.float32):
    ...
    upper2 = np.vstack(
        (np.full([2, Field.shape[1]], -999.99, dtype=fmt), Field[:-2, :])
    )

    midpoint = np.where(
        (Field > -400.0) & (upper < -400.0), Field - (lower - Field) / 2.0, -999.99
    )
    midpoint = np.where(
        (Field > -400.0) & (upper > -400.0), (upper + Field) / 2.0, midpoint
    )
    midpoint = np.where(
        (Field < -400.0) & (upper > -400.0), upper + (upper - upper2) / 2.0, midpoint
    )
    return midpoint
...
def cosine_of_solar_zenith_angle(Lons, Lats, DateTime, MinCosTheta=0.0, fmt=np.float32):
    ...
    try:
        ...
        for idx in range(CenMer.shape[0]):
            HourDiff = (
                np.where(
                    (Lons >= CenMer[idx] - 7.5) & (Lons < CenMer[idx] + 7.5),
                    HourVar[idx],
                    0.0,
                )
                + HourDiff
        ...
        Omega = (np.pi / 12.0) * (
            ...
            + HourDiff
            - 12.0
            + ((Lons - HourDiff * 15.0) / 15.0)
            + EqTime / 60.0
        )  # Hour angle, is in radians.
        CosTheta = np.sin(Delta) * np.sin(Lats * np.pi / 180.0) + np.cos(
            Delta
        ) * np.cos(Lats * np.pi / 180.0) * np.cos(
            Omega
        ...
        dict_CosTheta = {
            ...
            "standard_name": "cosine of solar zenith angle",
            "units": None,
            "undef": np.nan,
            "axis": "YX",
            "dimensions": ("y", "x"),
...
def find_pixels_of_region(Lons, Lats, LLLon, URLon, LLLat, URLat):
    ...
    try:
        ...
        Mask = np.where(
            (Lons >= LLLon) & (Lons <= URLon) & (Lats >= LLLat) & (Lats <= URLat),
            True,
            False,
        )
        yx = np.argwhere(Mask)
...
def create_gridmap(Domain, PixResol=2.0, fmt=np.float32):
    ...
    dict_Lons = {
        "long_name": "Longitude of gridmap",
...
def locate_files(
    path, prefix, datetime_ini, datetime_fin, use_parameter="scan_start_time"
):
    """
    ...
    try:
        assert (
            isinstance(datetime_ini, datetime.datetime)
            or (isinstance(datetime_ini, str) and len(datetime_ini) == 15)
        ) and (
            isinstance(datetime_fin, datetime.datetime)
            or (isinstance(datetime_fin, str) and len(datetime_fin) == 15)
        )
    except AssertionError:
        print(
        ...
        if use_parameter == "scan_start_time":
            ...
            mask = np.where(
                (ini_datetime >= datetime_ini) & (ini_datetime < datetime_fin),
                True,
                False,
            )
            ...
            mask = np.where(
                (fin_datetime >= datetime_ini) & (fin_datetime < datetime_fin),
                True,
                False,
            )
            files = np.array(files)
            ...
            fin_datetime = np.array(fin_datetime)
            mask = np.where(
                (ini_datetime >= datetime_ini) & (fin_datetime < datetime_fin),
                True,
                False,
            )
            files = np.array(files)
...
def accumulate_in_gridmap(
    ...
    try:
        assert (
            isinstance(parameter_lon, GOES) and isinstance(parameter_lat, GOES)
        ) or (
            isinstance(parameter_lon, np.ndarray)
            and isinstance(parameter_lat, np.ndarray)
        )
    except AssertionError:
        ...
        try:
            assert (isinstance(Lons, GOES) and isinstance(Lats, GOES)) or (
                isinstance(Lons, np.ndarray) and isinstance(Lats, np.ndarray)
            )
        except AssertionError:
            print("\nLons and Lats must be GOES class or numpy.ndarray\n")
            return
            ...
            try:
                assert (
                    isinstance(parameter_value, GOES)
                    or isinstance(parameter_value, np.ndarray)
                    or isinstance(parameter_value, type(None))
                )
            except AssertionError:
                ...
                if Ltng_Lon.shape[0] == 0:
                    ...
                    for j in range(len(yidx) - 1):
                        for i in range(len(xidx) - 1):
                            LonsCut = Lons[
                                yidx[j] : yidx[j + 1] + 1, xidx[i] : xidx[i + 1] + 1
                            ]
                            LonsCutMin, LonsCutMax = LonsCut.min(), LonsCut.max()
                            LonsCutCen = LonsCen[
                                yidx[j] : yidx[j + 1], xidx[i] : xidx[i + 1]
                            ]
                            ...
                            if Ltng_Lon2.shape[0] > 0:
                                ...
                                for k in range(len(zidx) - 1):
                                    ...
                                    Dist = np.sqrt(
                                        (LonsCutCen - Ltng_Lon3[:, None, None]) ** 2
                                        + (LatsCutCen - Ltng_Lat3[:, None, None]) ** 2
                                    )
                                    Mins = np.min(
                                        Dist.reshape(
                                            (
                                                Dist.shape[0],
                                                Dist.shape[1] * Dist.shape[2],
                                            )
                                        ),
                                        axis=1,
                                    )[:, None, None]
