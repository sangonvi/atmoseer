                        ...
                        print(
                            "  {} {:3.0f}% {:.1f}MB {}".format(
                                ...
                                (
                                    "{}m{}s".format(
                                        round(
                                            (datetime.now() - StartTime).seconds / 60.0
                                        ),
                                        (datetime.now() - StartTime).seconds % 60,
                                    )
                                    if (datetime.now() - StartTime).seconds > 60
                                    else "{}s".format(
    ...
    while DateTimeIniLoop < DateTimeFinLoop:
        ...
        for line in ListFiles:
            if Product[:-1] in ["ABI-L1b-Rad", "ABI-L2-CMIP"]:
                ...
                DateTimeFile = datetime.strptime(
                    NameFile[NameFile.find("_s") + 2 : NameFile.find("_e") - 1],
                    "%Y%j%H%M%S",
                )

                if (
                    Product2 in NameFile
                    ...
                    if not rename_fmt:
                        ...
                        NameOut = (
                            NameFile[: NameFile.find("_s") + 2]
                            + DateTimeFile.strftime(rename_fmt)
                            + ".nc"
                        )
